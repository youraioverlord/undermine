...
> **Implementation note:** the shipped game draws every sprite and tile **procedurally** — rectangles and blinks in the fixed palette, straight from the render code — rather than from authored PNG assets. The Aseprite pipeline and build-time palette validator in §3.2 were therefore never needed; the palette, size and color-count rules above govern the drawing code directly. §3.2 becomes relevant only if the art ever moves to authored sprite sheets.
...
> **Status: ✅ all six tracks are in** ([synth.c](src/synth.c)). The title theme is 32 rows of melody + triangle bass + a third **arpeggio voice** doing the SID chord trick (fast root/third/fifth cycling at ~50 Hz; a negative root in the pattern encodes a minor chord). The in-game loop stays two-voice. The depth variation lands every 10 rooms: a semitone transpose **plus a 1 s low-pass sweep** dipping to ~250 Hz and back. Death and room-clear play as prioritized SFX jingles. Game over is a one-shot minor-key resolution of the title line — the pattern player supports non-looping songs, so it says its piece and goes quiet until the title returns. Patterns live as C arrays (per-row Hz); the GoatTracker route was never needed.
...
> **Status: ✅ all implemented**, including the crusher slam (a two-stage noise thud fired the exact tick a piston bottoms out on your screen) and the death-jingle duck (music drops to 25% and fades back in over ~0.8 s).
...
(period `120 - 2*difficulty` ticks, floor 60). Coal per room: **9–15, uniform**
(depth makes rooms *harder to traverse*, not longer — the quota never grows).
...
> **Implemented** (`sim_cull_blocking_crushers`, run after enemy placement): the checker is re-run with each crusher's slam-column standing cell **banned from the path** (banning beats walling the tile off, which the BFS would just stand on top of); if the room stops being solvable, that crusher is deleted. Coal inside a kill zone is set aside for the probe (a guarded lump is a timing puzzle, not a wall); a key/lamp there skips the probe for the same reason. Note that with the current open platform-chain generator the checker's jump envelope can hop past any single banned cell, so in practice the rule almost never fires — it is the safety net this section asks for, and it matters the day generation gets more confined or organic.
...
> **Implemented as render-side theater** (`draw_screen_fx`): on a descent a ~0.65 s black curtain unveils the new room top-down (with a DEPTH card), as if dropping in through the entry shaft; a death strobes the screen white-then-red for the first dying ticks before the sprite's own death anim plays out. The sim is never paused for either, which keeps the two lock-step screens, replays and determinism untouched — each screen runs its own effect when *its* mole descends or dies.
...
Coal = 25 pts, room clear = 100 + depth×10, key = 50. HUD (top char row): score, lives (mole icons), coal `n/quota`, depth. Leaderboard ("deepest dives") of 20 entries, persisted locally: each row is a name (the human is `YOU`; bots use their name label), the depth reached, and the deaths that run took. Ordered depth-descending, deaths-ascending; one row per name (a returning name must beat its own best). Entries are recorded when a run ends: human game over (records the human and the right-screen bot) and demo arena end (records both bots). The score file is untrusted input: it is sanitized on load (names NUL-terminated and cut at the first non-printable byte, numbers clamped) so a corrupt or hand-edited file can never crash the renderer.

### 6.6 Air (oxygen)

A per-run air tank that lives in the **shell**, not the deterministic sim: **2.5 minutes** of 50 Hz ticks, drawn as a HUD bar. Reaching an exit refills it — in the demo either bot's descent counts (one solver keeps the arena alive), in a human run only the player's own descent does. Running dry suffocates the human (`sim_kill_player`: lose a life, restart the room, fresh tank with the new life); in the demo it reseeds the arena (neither bot found an exit in 2.5 minutes — the mine has beaten them, both bots go on the leaderboard first). It exists to bound both a dawdling run and a stalled bot without putting any wall-clock timer inside the sim.
...
> **TODO — none of these ship as runtime asserts yet.** Several are covered *indirectly* (the sprite budget by generation caps + a property test, room validation by the 1200-room suite, the palette by procedural drawing that only indexes `C64[]`), but there is no debug build with live guards. Cheap to add; do it before any generator rework.
...
5. **Golden seeds:** 20 pinned seeds with snapshot-tested room layouts, so intentional generator changes show up as reviewed diffs, not silent shifts. **TODO — not yet implemented** (the property suite catches *broken* rooms but not *silently changed* ones).
6. **Performance budget:** full tick + room generation under 2 ms on target hardware (laughably easy in C, but the assert catches accidental O(n²)). **TODO — not yet implemented.**
...
3. **Danger (week 3):** ✅ *Done* (bar the solver-bot navigator, tracked as future work in §7.2.4). All five enemies implemented, deterministic (integer timers / triangle waves, no float `sin`; any "random" enemy choice draws from a seeded PRNG stored *in* `GameState` and advanced only during `tick`, so replays and the daily seed stay bit-identical): crusher (ceiling piston, depth ≥2 — depth 1 is the safe dark tutorial room), foreman (roams platforms + climbs ladders, **picking a random continuation at each ladder junction** and a random heading when it lands; also **steals coal** — grabs a lump it walks over, hauls it a few seconds, then drops it on another platform cell, so the layout of what you still have to collect keeps shifting — though it never grabs the *last* ground lump; depth ≥4), bat (diagonal sweep bouncing off terrain, depth ≥7), spider (bobs on a ceiling thread, depth ≥10), boulder (rolls along platforms, falls off edges, depth ≥13). Depth-gated placement + difficulty ramp per §5.4, spawn-safe zone, ≤8-sprite budget, one-hit contact death (hitboxes ~70% for fairness; a crusher kills on touch even while idle). Depth 15 is the **menagerie**: one of every enemy type in one room. Solver-bot integration test (§7.2.4) implemented as a partial oracle — it drove out a head-clearance bug in the checker + physics; clears ~67% of rooms, gated as a regression floor. **Special rooms done:** key rooms (depth ≥10, ~1 in 4) hide a key the exit also requires, with a HUD indicator; dark rooms (depth 1 always — the tutorial teaches the lamp — then occasionally from depth 20) black out everything beyond a square of sight around Monty until he grabs a lamp. Both pickups are placed on reachable platform cells (the checker verifies reachability), and darkness is a pure render effect so determinism holds. The **crusher-path-safety rule** (§5.5) is in (`sim_cull_blocking_crushers`). **Still to do:** a fully robust solver-bot navigator (future work).
4. **Sound (week 4):** ✅ *Done.* Tiny SID-style software synth ([synth.c](src/synth.c)): 3 voices, pulse/triangle/saw/noise waveforms, LFSR noise, per-segment attack/release envelopes (no clicks). SFX wired to game events (jump, land, coal pickup, quota-met, death, descend, crusher slam) with priority-based voice stealing. The full §4.2 composition plan is in: a 32-row title theme with melody + bass + a third arpeggio-chord voice, the sparse in-game loop, the every-10-depths semitone transpose **plus low-pass filter sweep**, and a one-shot minor-key game-over jingle (the pattern player supports non-looping songs); the death jingle ducks the music and fades it back. The whole synth lives in the output layer (never touches `GameState`, so determinism holds) and its sample generation is device-free, so it's unit-tested headlessly: silent-when-idle, audible-when-triggered, self-terminating, music-loops-audibly, music-leaves-SFX-voices-free, SFX-over-music, jingle-plays-once, duck-arms-and-releases, sweep-stays-clean, slam-thuds. **Optional later:** swap to reSID if the music ever warrants it.
5. **Shell (week 5):** ✅ *Done.* State machine per §6.1: `TITLE → PLAYING ⇄ PAUSED`, `PLAYING → GAME_OVER → TITLE`. Title screen with the leaderboard; a game-over screen (with the one-shot game-over jingle); the 20-entry **deepest-dives** leaderboard (§6.5) persisted to a local file — no name-entry screen, rows carry the player's `YOU` / the bots' name labels automatically. Three start modes: a normal run (time-seeded), a **daily run** (**D** — seed derived from the date, so everyone gets the same mine that day), and a typed 6-hex-digit seed (**S** — the current seed shows in-game, so a good mine can be shared and replayed). **F2/F3** load MoleScript bots via a file picker (Windows; `--bot1`/`--bot2` on any platform). Pause on P; ESC quits a run to the title, and the title to the desktop; M on the title toggles sound (the synth keeps rendering while muted, so unmuting rejoins the music in time). **Attract/demo mode:** after ~10s idle on the title the solver-bot takes over and plays the live game (a blinking "DEMO" banner; any key returns to the title). The bot's per-tick brain (`bot_decide`) is shared with the headless solver-bot test, so the same navigator drives both; a watchdog reseeds the demo if the bot stalls or dies out, so attract mode never hangs. For the live game the demo bot adds three reactive safety layers on top of the shared navigator:
...
It's a heuristic, not a perfect player — fast bats and crushers still catch it occasionally and some rooms it simply can't solve (it reseeds past those) — but it descends steadily and almost never falls to its death. The sim now raises `PS_GAMEOVER` when lives run out (instead of silently resetting) and the shell drives the flow from there — the time/date seeding lives in the shell, never in the deterministic sim. The polish pass has landed too: the death **palette flash** and the **shaft-drop room-transition reveal** (both render-side, see §6.1) and the title theme's third-voice rework (§4.2). Both bots also narrate what they're doing — a `say` line beside each name, streamed to `bot1.log` / `bot2.log` per run — whether driven by the built-in C brains or a MoleScript.
